...
const STATUS: Record<Task["status"], { label: string; icon: typeof Circle; color: string; bg: string }> = {
...
const FILTERS = ["all", "open", "in_progress", "done"] as const;
    ...
    <div className={`h-full flex flex-col bg-background ${fullPage ? "rounded-[28px] border border-border/70 shadow-[0_18px_50px_rgba(15,23,42,0.08)]" : "border-l border-border w-80 shrink-0"}`}>
      ...
      <div className="px-3 py-2 border-b border-border flex gap-2 shrink-0">
        <input
          ...
          onKeyDown={e => { if (e.key === "Enter" && newTitle.trim()) { onCreateTask(newTitle.trim()); setNewTitle(""); } }}
        ...
        />
        <button onClick={() => { if (newTitle.trim()) { onCreateTask(newTitle.trim()); setNewTitle(""); } }} disabled={!newTitle.trim()}
      ...
      <div className="flex border-b border-border shrink-0">
        {FILTERS.map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`flex-1 text-[11px] py-2 font-medium transition-colors ${filter === f ? "text-primary border-b-2 border-primary" : "text-muted-foreground hover:text-foreground"}`}>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2">
        <AnimatePresence>
          {filtered.map(task => {
            ...
            return (
              <motion.div key={task.id} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }}
                className="bg-card border border-border rounded-xl p-3 flex items-start gap-2.5">
                <button onClick={() => cycleStatus(task)} className={`mt-0.5 shrink-0 ${cfg.color} hover:opacity-70 transition-opacity`} title="Click to advance status">
